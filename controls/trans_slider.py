...
class TransSlider(QWidget):
    ...
    def __init__(self, idx, min_trans, max_trans, initial_trans, vert_stack = False, label_to_side = True):
        ...
        for id_, slider in enumerate(self.sliders):
            ...
            if id_ == 0:
                trans_axis = QLabel("X Axis")
            elif id_ == 1:
                trans_axis = QLabel("Y Axis")
            else:
                trans_axis = QLabel("Z Axis")
            horiz_layout.addWidget(trans_axis)
            horiz_layout.addWidget(QLabel(" "))
